...
export default function RegionLanding() {
  ...
  const { region, isLoading } = useRegion(regionSlug);
  ...
  if (!region) return <View style={styles.container}><Text style={styles.loading}>Region not found</Text></View>;

  const sections = [
    {
      label: 'Meeting Minutes',
      count: stats.minutes,
      href: routes.region.minutes.index(regionSlug),
      icon: '📋',
    },
    {
      label: 'Ordinances',
      count: stats.ordinances,
      href: routes.region.ordinances.index(regionSlug),
      icon: '📜',
    },
    {
      label: 'Contact Directory',
      count: stats.contacts,
      href: routes.region.contacts.index(regionSlug),
      icon: '📞',
    },
    {
      label: 'Elections',
      count: 0,
      href: routes.region.elections.index(regionSlug),
      icon: '🗳️',
    },
  ];
