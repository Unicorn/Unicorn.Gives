...
export default function OrdinanceDetailRedirect() {
  const router = useRouter();
  const { slug } = useLocalSearchParams<{ slug: string }>();

  const target = slug
    ? `/county/${COUNTY_SLUG}/townships/${TOWNSHIP_SLUG}/ordinances/${encodeURIComponent(slug)}`
    : `/county/${COUNTY_SLUG}/townships/${TOWNSHIP_SLUG}/ordinances`;
  ...
  return (
    <View style={styles.c}>
      <Text style={styles.t}>Redirecting to Ordinance Detail…</Text>
      <Link href={target} asChild>
        <TouchableOpacity style={styles.link} activeOpacity={0.85}>
          <Text style={styles.linkText}>Open ordinance</Text>
        </TouchableOpacity>
      </Link>
    </View>
  );
