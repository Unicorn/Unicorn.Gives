...
export default function OrdinancesRedirect() {
  const router = useRouter();
  const target = `/county/${COUNTY_SLUG}/townships/${TOWNSHIP_SLUG}/ordinances`;
  ...
  return (
    <View style={styles.c}>
      <Text style={styles.t}>Redirecting to Ordinances…</Text>
      <Link href={target} asChild>
        <TouchableOpacity style={styles.link} activeOpacity={0.85}>
          <Text style={styles.linkText}>Open ordinances</Text>
        </TouchableOpacity>
      </Link>
    </View>
  );
