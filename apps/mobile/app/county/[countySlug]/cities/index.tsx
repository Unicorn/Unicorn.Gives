...
export default function CitiesList() {
  ...
  useEffect(() => {
    if (!region) return;
    supabase.from('regions').select('slug, name, type').eq('parent_id', region.id).eq('type', 'city').eq('is_active', true).order('name')
      .then(({ data }) => { if (data) setItems(data); });
  }, [region]);

  return (
    <View style={{ flex: 1 }}>
      <AppHeader title="Cities" showBack />
      <ScrollView style={{ flex: 1, backgroundColor: '#fcf9f4' }} contentContainerStyle={{ padding: 16, paddingBottom: 40 }}>
        {items.length === 0 && <Text style={{ color: '#73796d' }}>No cities listed yet.</Text>}
        {items.map((r) => (
          <Link key={r.slug} href={routes.county.cities.municipalRoot(countySlug, r.slug)} asChild>
