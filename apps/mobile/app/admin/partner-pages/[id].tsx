...
export default function EditPartnerPagePage() {
  const { id } = useLocalSearchParams<{ id: string }>();
  const router = useRouter();
  const { colors } = useTheme();
  const styles = useMemo(() => createStyles(colors), [colors]);

  const { update, remove, publish, unpublish, archive, loading: mutating } = useAdminMutation('partner_pages');
  ...
  async function handleSave() {
    ...
    if (success && Platform.OS === 'web') {
      window.alert('Partner page saved successfully.');
    }
  }
  ...
  async function handleDelete() {
    ...
    if (success) {
      router.replace(toHref('/admin/partner-pages'));
    }
    setShowDelete(false);
  }
  ...
  if (loading) {
    return (
      <View style={styles.loadingContainer}>
        ...
        <Text style={styles.loadingText}>Loading partner page...</Text>
      </View>
    );
  }
