...
export default function OrdinancesListPage() {
  ...
  const { data, loading, error, total, pageSize, refresh } = useAdminQuery<OrdinanceRow>(
    'ordinances',
    {
      select: 'id, slug, title, number, category, status, adopted_date, created_at',
      orderBy: 'created_at',
      ascending: false,
      page,
      pageSize: 25,
  ...
  const columns: Column<OrdinanceRow>[] = [
    ...
    },
    {
      ...
      width: 90,
      render: (row) => (
        <Text style={styles.dateCell}>{row.number ?? '—'}</Text>
      ),
    },
    ...
    <AdminPageShell
      ...
      actions={
        <AdminButton
          label="New Ordinance"
          icon="add"
          onPress={() => router.push(toHref('/admin/ordinances/new'))}
        />
      }
    >
      {/* Filters row */}
