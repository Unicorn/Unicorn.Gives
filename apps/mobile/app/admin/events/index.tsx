...
export default function EventsListPage() {
  ...
  const { data, loading, error, total, pageSize, refresh } = useAdminQuery<EventRow>(
    'events',
    {
      select: 'id, slug, title, date, location, category, visibility, region_id, regions(name), status',
      orderBy: sortKey,
      ascending: sortAsc,
      page,
      pageSize: 25,
  ...
  const { remove } = useAdminMutation('events');
  ...
  const columns: Column<EventRow>[] = [
    ...
    { key: 'visibility', label: 'Visibility', width: 90 },
  ...
  return (
    <AdminPageShell
      ...
      actions={<AdminButton label="New Event" icon="add" onPress={() => router.push(toHref('/admin/events/new'))} />}
    >
      <View style={styles.filtersRow}>
        <TextInput style={styles.searchInput} value={search}
          onChangeText={(text) => { setSearch(text); setPage(1); }}
          placeholder="Search events..." placeholderTextColor={colors.outlineVariant} />
      ...
      <AdminDataTable
        ...
        onRowPress={(row) => router.push(toHref(`/admin/events/${row.id}`))}
      ...
      <AdminConfirmDialog
        visible={!!deleteTarget} title="Delete Event"
