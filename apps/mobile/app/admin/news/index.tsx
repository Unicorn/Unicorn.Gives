...
export default function NewsListPage() {
  ...
  const { data, loading, error, total, pageSize, refresh } = useAdminQuery<NewsRow>(
    'news',
    {
      select: 'id, slug, title, date, category, visibility, region_id, regions(name), status',
      orderBy: sortKey,
      ascending: sortAsc,
      page,
      pageSize: 25,
  ...
  return (
    <AdminPageShell
      ...
      actions={<AdminButton label="New Article" icon="add" onPress={() => router.push(routes.admin.newNews())} />}
    >
      <View style={styles.filtersRow}>
        <TextInput style={styles.searchInput} value={search}
          onChangeText={(text) => { setSearch(text); setPage(1); }}
          placeholder="Search articles..." placeholderTextColor={colors.outlineVariant} />
      ...
      <AdminConfirmDialog
        visible={!!deleteTarget} title="Delete Article"
