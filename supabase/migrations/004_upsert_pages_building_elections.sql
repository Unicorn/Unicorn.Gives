-- =============================================================================
-- Unicorn.Gives Content Page Bootstrap
-- =============================================================================
-- Purpose:
--   Ensure these `public.pages` rows exist on a blank DB:
--   - building
--   - elections
--
--   `003_update_pages_building_elections.sql` uses UPDATE statements, which
--   do nothing when `public.pages` is empty. This migration uses INSERT
--   with `ON CONFLICT (slug) DO UPDATE` so it is safe to run repeatedly.
...
VALUES
  (
    'building',
    'Lincoln Township Building Department',
    'Building permits, inspections, and construction requirements for Lincoln Township',
    $pages_body_building$
## Building Department
...
VALUES
  (
    'elections',
    'Lincoln Township Election Information',
    'Voter registration, absentee ballots, and election information for Lincoln Township residents',
